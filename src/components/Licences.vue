...
export default {
  ...
  methods: {
      ...
      proxy //Get data through cors proxy
        ...
        .then(response => {
          ...
          const expiringLicenes = licences.filter(lic => {
            // filter to display licences that will expire in the next two months
          ...
          expiringLicenes.sort(function(a, b) {
            //Sort expiring licences in order of the closest to expire. Set dates to a single case and sort.
    ...
    fakeValues() {
      // Generate Dummy Values to populate list DELETE WHEN I'M GONE
      let licence2 = [
        {
    ...
    updateValues(expiringLicences) {
      ...
      localStorage.setItem(
        "Expiring Licences",
        JSON.stringify(expiringLicences)
      );
      localStorage.setItem(
        "No of Expiring Licences",
        JSON.stringify(expiringLicences.length)
      );
    }
  }
};
